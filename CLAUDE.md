...
- **IPC is worker-initiated only**: The worker writes questions; the dispatcher writes answers. The dispatcher must never write unsolicited files to `ipc/` — the worker only polls for `.answer` files matching its own `.question` files. To pass additional context to a running worker, append a note to the plan file instead; the worker reads the plan file as it progresses through checklist items.
